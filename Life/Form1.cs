...
namespace Life
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            splitDate = dateTimePicker1.Text.Split('年', '月', '日');
            if (splitDate[1] == "1")
            {
                if (int.Parse(splitDate[2]) <= 20)
                    dictionary = CreateCapricorn();
                else
                    dictionary = CreateAquarius();
            }
            else if (splitDate[1] == "2")
            {
                if (int.Parse(splitDate[2]) <= 19)
                    dictionary = CreateAquarius();
                else
                    dictionary = CreatePieces();
            }
            ...
            else if (splitDate[1] == "5")
            {
                if (int.Parse(splitDate[2]) <= 20)
                    ...
                    dictionary = CreateGemini();
            }
            else if (splitDate[1] == "6")
            {
                if (int.Parse(splitDate[2]) <= 21)
                    dictionary = CreateGemini();
                else
                    dictionary = CreateCancer();
            }
            else if (splitDate[1] == "7")
            {
                if (int.Parse(splitDate[2]) <= 22)
                    dictionary = CreateCancer();
                else
                    dictionary = CreateLeo();
            }
            ...
            else if (splitDate[1] == "9")
            {
                if (int.Parse(splitDate[2]) <= 22)
                    ...
                    dictionary = CreateLibra();
            }
            else if (splitDate[1] == "10")
            {
                if (int.Parse(splitDate[2]) <= 23)
                    dictionary = CreateLibra();
                else
                    dictionary = CreateScorpio();
            }
            ...
            A = int.Parse(splitDate[0]) / 1000;
            B = int.Parse(splitDate[0]) % 1000 / 100;
            C = int.Parse(splitDate[0]) % 100 / 10;
            D = int.Parse(splitDate[0]) % 10;
            E = int.Parse(splitDate[1]) / 10;
            F = int.Parse(splitDate[1]) % 10;
            G = int.Parse(splitDate[2]) / 10;
            H = int.Parse(splitDate[2]) % 10;

            var life = A + B + C + D + E + F + G + H;
            int temp1 = 0, temp2 = 0;
            while (life >= 10)
            {
                temp1 = life / 10;
            ...
            var result = dictionary.Where((x) => x.Key == life);
            foreach (var i in result)
                labResult.Text = "你的生命靈數" + i.Key.ToString() + "：" + i.Value.ToString();
        }
